 ...
 *
 * Author: Lonelyoneskatter <threesixoh.skater@yahoo.com>
 *
 */
...
#define DEFAULT_SCREEN_OFF_MAX 960000
 ...
 * function.
 */
static void cpufreq_skateractive_timer_start(int cpu)
{
	struct cpufreq_skateractive_cpuinfo *pcpu = &per_cpu(cpuinfo, cpu);
...
static unsigned int choose_freq(
	struct cpufreq_skateractive_cpuinfo *pcpu, unsigned int loadadjfreq)
{
...
rearm:
	if (!timer_pending(&pcpu->cpu_timer))
		cpufreq_skateractive_timer_resched(data, false);
...
static void cpufreq_skateractive_idle_end(void)
{
	...
	if (!timer_pending(&pcpu->cpu_timer)) {
		cpufreq_skateractive_timer_resched(smp_processor_id(), false);
...
static struct attribute *skateractive_attributes[] = {
...
static struct attribute_group skateractive_attr_group = {
	.attrs = skateractive_attributes,
	.name = "skateractive",
};
		...
		unsigned int event)
{
	...
	case CPUFREQ_GOV_START:
		...
		for_each_cpu(j, policy->cpus) {
			...
			pcpu->last_evaluated_jiffy = get_jiffies_64();
			cpufreq_skateractive_timer_start(j);
	...
	case CPUFREQ_GOV_LIMITS:
		...
		for_each_cpu(j, policy->cpus) {
			...
			if (policy->min < pcpu->min_freq)
				cpufreq_skateractive_timer_resched(j, true);
			pcpu->min_freq = policy->min;
...
#ifndef CONFIG_CPU_FREQ_DEFAULT_GOV_SKATERACTIVE
static
#endif
...
MODULE_DESCRIPTION("'cpufreq_skateractive' - A cpufreq governor for "
