...
get_emmc_wp_status(struct mmc_card *card, unsigned int addr, void *buf)
{
	...
	pr_err("get_emmc_wp_status()::e,addr=%d\n",addr);
	...
	pr_err("get_emmc_wp_status()::e,wp_grp_size=%d, addr=%d, cmd.arg=%d\n",wp_grp_size, addr, cmd.arg);
	...
	for(/*i = 1*/; i < MAX_PARTION_NUMBER; i++){
	    ...
	    if (p) {
	        if (p->info && p->info->volname[0]){
	            ...
	            pr_err("set_partition_ro()::e,wp_status_buf[7]=0x%x\n",wp_status_buf[7]);
	            if((wp_status_buf[7] & 0x3) == POWER_ON_WP_STATUS){
	                p ->policy = 1;
