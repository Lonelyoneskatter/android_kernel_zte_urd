				...
				unsigned char *data, int size)
{
  ...
  if (chip && (i2c = chip->bus) && data && (size <= MAX_WRITE_ARRAY_SIZE))
  {
...
static void sx9500_process_interrupt(psx9500_chip_t chip,u8 nirqlow)
{
  ...
  int counter = 0;
  if (!chip) {
    printk(KERN_ERR "%s, NULL sx86XX_t\n", __func__);
    return;
  }
...
//      input->id.version = sx863x->version;
	...
	if (IS_ERR_OR_NULL(chip->pinctrl)) {
		//printk("%s, error devm_pinctrl_get(), chip->pinctrl 0x%x\n", __func__, (unsigned int)chip->pinctrl);
		goto probe_error;
	}
	else
	{
		...
		{
			printk("%s, error pinctrl_lookup_state() for SSD1306B_ACTIVE\n", __func__);
			goto probe_error;
		}
		chip->sx9500_sleep= pinctrl_lookup_state(chip->pinctrl, SSD1306B_SLEEP);
		if (IS_ERR_OR_NULL(chip->sx9500_sleep))
		{
			printk("%s, error pinctrl_lookup_state() for SSD1306B_SLEEP\n", __func__);
			goto probe_error;
		}
	}
	...
	if (IS_ERR(chip->sx9500_vdd)) {
		printk("unable to get sx9500 vdd\n");
	...
	if (IS_ERR(chip->sx9500_vdd)) {
		printk("unable to get sx9500 svdd\n");
