		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		switch (button){
			case 0:
				//x=1118*320/50;
				x=syn_ts->sensor_max_x*160/740;
				break;
			case 1:
				x=syn_ts->sensor_max_x*375/740;
				break;
			case 2:
				x=syn_ts->sensor_max_x*580/740;
				break;
			}
		y=syn_ts->sensor_max_y*(1335+75)/1335;
...
printk("pzh:enter synaptics_rmi4_detection_work\n");
	mutex_lock(&exp_data.mutex);
	if (!list_empty(&exp_data.list)) {
		list_for_each_entry_safe(exp_fhandler,
				...
				&exp_data.list,
				link) {
				printk("pzh:exp_fhandler->fn_type=%d\n",exp_fhandler->fn_type);
			if ((exp_fhandler->func_init != NULL) &&
					(exp_fhandler->inserted == false)) {
					printk("pzh:enter exp_fhandler->func_init\n");
	...
	mutex_unlock(&exp_data.mutex);
printk("pzh:leave synaptics_rmi4_detection_work\n");
	return;
}
		...
		bool *was_in_bl_mode)
{
	...
	printk("pzh:Enter synaptics_rmi4_check_status\n");
	...
	if (retval < 0) {
		/*dev_err(dev_err,
				...
				__func__);*/\
	       printk("pzh:Failed to read interrupt status\n");
		return retval;
	}
...
void zte_synaptics_change_fw_config( struct synaptics_rmi4_data *ts )//xym
{
	...
	{
		int retval=0;
		printk("%s: xym, change the ecw fw config!\n", __func__); 
		
		retval=synaptics_rmi4_i2c_write(ts,0x78,success_data0,1); //F11_2D_CTRL47  
	...
	printk("sensor:0x%x",ts->config_id.sensor);
	if(ts->config_id.sensor==0x31){//TPK	
    	printk("notice:change the TPK fw config!\n");
	...
	if(ts->config_id.sensor==0x37){//LAIBAO
    	printk("notice:change the LAIBAO fw config!\n");
    	i2c_smbus_write_byte_data(ts->i2c_client,0xff,1);
...
{
	printk("hjy synaptics_rmi4_init\n");
