...
#include <soc/qcom/socinfo.h>//zte_pm liyf 20141124
#ifdef CONFIG_VFP
#include <asm/vfp.h>
#endif
#include "idle.h"
...
#include <linux/seq_file.h>     //zte_pm liyf 20141124
...
long lcd_on_time_s_before_enter_suspend = 0;  
void zte_update_lateresume_2_earlysuspend_time(bool resume_or_earlysuspend)	// LHX_PM_20110411_01 resume_or_earlysuspend? lateresume : earlysuspend
{
	...
	else	//earlysuspend,need to record when the lcd is turned off
	{
        ...
        lcd_on_time_s_before_enter_suspend += lateresume_2_earlysuspend_time_s;
...
*/
		...
		if(!record_firsttime)
			{
				...
				time_lcdon_total_s += lcd_on_time_s_before_enter_suspend;
			}
		...
		pr_info("[ZTE_PM] APP wake for %6d.%03d s, lcd on for %6d s %3d %%,modem wake for %10d ms(%s) %4d %%o,modem sleep for %10d ----%d%d\n", 
			time_updated_when_sleep_awake_s,time_updated_when_sleep_awake_ms,(int)lcd_on_time_s_before_enter_suspend,(int)(lcd_on_time_s_before_enter_suspend*100/(time_updated_when_sleep_awake_s + 1))
		...
		lcd_on_time_s_before_enter_suspend = 0;	//LHX_PM_20110411_01 clear how long the lcd keeps on
